        ...
        if (VAO == 0)
        {
            ...
            for (unsigned int x = 0; x <= X_SEGMENTS; ++x)
            {
                for (unsigned int y = 0; y <= Y_SEGMENTS; ++y)
                {
                    ...
                    // here I have multiplied by .5 to make the x axis long, and .05 on the z and y axes since I want the object to be very narrow and thin
                    float xPos = std::cos(xSegment * 2.0f * PI) * std::sin(ySegment * PI) * .5 + 0;
                    float yPos = std::cos(ySegment * PI) * .025 + 1.23;
                    float zPos = std::sin(xSegment * 2.0f * PI) * std::sin(ySegment * PI) * .025 + 1.15;
