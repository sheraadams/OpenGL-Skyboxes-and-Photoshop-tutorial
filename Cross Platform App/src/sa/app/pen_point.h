        ...
        if (VAO == 0)
        {
            ...
            for (unsigned int x = 0; x <= X_SEGMENTS; ++x)
            {
                for (unsigned int y = 0; y <= Y_SEGMENTS; ++y)
                {
                    ...
                    // the numbers here are used to create a very small pen point object
                    float xPos = std::cos(xSegment * 2.0f * PI) * std::sin(ySegment * PI) * .033 + -.478;
                    float yPos = std::cos(ySegment * PI) * .0033 + 1.23;
                    float zPos = std::sin(xSegment * 2.0f * PI) * std::sin(ySegment * PI) * .0033 + 1.15;
