        ...
        { // generate VBO, VAO, EBO
            ...
            for (unsigned int x = 0; x <= X_SEGMENTS; ++x)
            {
                for (unsigned int y = 0; y <= Y_SEGMENTS; ++y)
                {
                    ...
                    // this is a sphere shape
                    // the multiplication elongates or truncates the shape which is not used here.
                    // the addition or subtraction shifts the shape up and down like a translation
                    // here I have multiplied by .0083 since I want the objefct to be very very small
                    float xPos = std::cos(xSegment * 2.0f * PI) * std::sin(ySegment * PI) * .0083 + .38;
                    float yPos = std::cos(ySegment * PI) * .0083 + 1.23;
